const CloudSync = {
    ...
    loadConfig() {
        try {
            const saved = Storage.getItem('gamehub_cloud_config');
            if (saved) {
    ...
    saveLocalDataVersion(version) {
        ...
        try { Storage.setItem('gamehub_local_data_version', version); } catch(e) {}
    },
  ...
  "projectId": "xxx"
    ...
    saveSettings() {
        ...
        if (firebaseUrl) {
            ...
            try { Storage.setItem('gamehub_firebase_url', firebaseUrl); } catch(e) {}
    ...
    async loadFromFirebaseUrl() {
        ...
        if (data !== null && data !== undefined) {
                ...
                this.saveConfig();
                try { Storage.setItem('gamehub_firebase_url', url); } catch(e) {}
    ...
    mapGameFields(game) {
        ...
        const defaultFieldOrder = [
            '文件ID', '备注', '百度', '迅雷', 'UC', '夸克', '预览', '排雷 评价', '排雷|评价',
    ...
    normalizeAllFields(games) {
        ...
        const defaultFieldOrder = [
            '文件ID', '类型', '游戏名', '备注', '百度', '迅雷', 'UC', '夸克', '预览', 
            '排雷|评价', '排雷 评价', '评级', '评级（成品级别）评分105- X          90-100 SSS           75-85 SS             60-70 S         45-55A     25-40B', 
            '剧情有无代入感10分，实用度如何，20分好不好冲？（30分）',
            '画风立绘建模如何？10分。动态？10分。cg质量？10分（30分）', 
            'CV质量10分，音声10分（20分）', '游戏性|玩法，好不好玩？（15分）', '内容cg丰富度（15分）', 
            '修正分，bug过多，挤牙膏，无意义刷量强行延长游玩时间+反作弊', '封面', '攻略', 'DL号|社团|作者', 
            '最后修改时间', '创建时间', '视频', '版本及更新时间等', '游戏名大小', '更新日志', '搜索', 'FB'
        ];
        ...
        try { Storage.setItem('gamehub_field_order', JSON.stringify(globalFields)); } catch(e) {}
